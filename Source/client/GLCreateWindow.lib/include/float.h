...
#define _controlfp __imported_name(_controlfp)
extern unsigned int	_controlfp (unsigned int unNew, unsigned int unMask);
#define _control87 __imported_name(_control87)
extern unsigned int	_control87 (unsigned int unNew, unsigned int unMask);


#define _clearfp __imported_name(_clearfp)
extern unsigned int	_clearfp ();	/* Clear the FPU status word */
#define _statusfp __imported_name(_statusfp)
extern unsigned int	_statusfp ();	/* Report the FPU status word */
#define		_clear87	_clearfp
#define		_status87	_statusfp

#define _fpreset __imported_name(_fpreset)
extern void		_fpreset ();	/* Reset the FPU */

/* Global 'variable' for the current floating point error code. */
#define __fpecode __imported_name(__fpecode)
extern int *	__fpecode();
...
#define _chgsign __imported_name(_chgsign)
extern double	_chgsign	(double x);
#define _copysign __imported_name(_copysign)
extern double	_copysign	(double dTo, double dFrom);
#define _logb __imported_name(_logb)
extern double	_logb		(double x);
#define _nextafter __imported_name(_nextafter)
extern double	_nextafter	(double x, double y);
#define _scalb __imported_name(_scalb)
extern double	_scalb		(double x, long n);

#define _finite __imported_name(_finite)
extern int	_finite		(double x);
#define _fpclass __imported_name(_fpclass)
extern int	_fpclass	(double x);
#define _isnan __imported_name(_isnan)
